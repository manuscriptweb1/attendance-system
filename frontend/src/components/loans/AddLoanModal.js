...
import { FiX, FiDollarSign, FiCheckCircle, FiLayers } from 'react-icons/fi';
              ...
              >
                ...
                {employees.map(emp => (
                  <option key={emp.id} value={emp.id}>
                    {emp.name} ({emp.employee_id || emp.id}) {emp.department_name ? `- ${emp.department_name}` : ''}
                  </option>
                ))}
              </select>
              {selectedEmp && (
                <div className="mt-2 text-xs bg-admin-bg/60 border border-admin-border p-2.5 rounded-xl flex items-center justify-between text-admin-text">
                  <span><strong>Code:</strong> {selectedEmp.employee_id || selectedEmp.id}</span>
                  <span><strong>Name:</strong> {selectedEmp.name}</span>
                  <span><strong>Dept:</strong> {selectedEmp.department_name || 'N/A'}</span>
                </div>
              )}
            </div>
