...
import { FiRefreshCw, FiHome, FiServer } from 'react-icons/fi';
  ...
  const handleRetry = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        setErrorMessage('Server is still unreachable. Please wait a moment.');
        setChecking(false);
      }
    } catch (err) {
      setTimeout(() => {
        setErrorMessage('Server is still not responding. Please try again in a few moments.');
        setChecking(false);
      }, 600);
    }
  };
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-[#FAFCFF] via-[#F8FAFC] to-[#F1F5F9] flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 py-8 sm:py-12 relative overflow-hidden font-sans select-none">
      ...
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center z-10">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 items-center mb-8 sm:mb-10">
          ...
          {/* Left Column: Server Stack Illustration */}
          <div className="md:col-span-6 flex justify-center items-center">
            <div className="relative w-48 h-48 xs:w-56 xs:h-56 sm:w-72 sm:h-72 md:w-80 md:h-80 flex items-center justify-center">
              ...
              {/* Custom SVG Illustration: Server Stack with Warning Triangle */}
              <svg className="w-36 h-36 xs:w-44 xs:h-44 sm:w-56 sm:h-56 md:w-64 md:h-64 z-10 filter drop-shadow-xl" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
                {/* Server Blade 1 (Top) */}
                <rect x="42" y="44" width="104" height="28" rx="8" fill="#DBEAFE" stroke="#93C5FD" strokeWidth="2.5" />
                <circle cx="58" cy="58" r="3.5" fill="#2563EB" />
                <circle cx="70" cy="58" r="3.5" fill="#2563EB" />
                <line x1="120" y1="58" x2="134" y2="58" stroke="#93C5FD" strokeWidth="3" strokeLinecap="round" />

                {/* Server Blade 2 (Middle) */}
                <rect x="42" y="80" width="104" height="28" rx="8" fill="#DBEAFE" stroke="#93C5FD" strokeWidth="2.5" />
                <circle cx="58" cy="94" r="3.5" fill="#2563EB" />
                <circle cx="70" cy="94" r="3.5" fill="#2563EB" />
                <line x1="120" y1="94" x2="134" y2="94" stroke="#93C5FD" strokeWidth="3" strokeLinecap="round" />

                {/* Server Blade 3 (Bottom) */}
                <rect x="42" y="116" width="104" height="28" rx="8" fill="#DBEAFE" stroke="#93C5FD" strokeWidth="2.5" />
                <circle cx="58" cy="130" r="3.5" fill="#2563EB" />
                <circle cx="70" cy="130" r="3.5" fill="#2563EB" />
                <line x1="120" y1="130" x2="134" y2="130" stroke="#93C5FD" strokeWidth="3" strokeLinecap="round" />

                {/* Red Warning Triangle Badge (Overlayed on bottom right of server) */}
                <g filter="drop-shadow(0px 4px 8px rgba(239, 68, 68, 0.35))">
                  <path
                    d="M142 108C145.464 102 154.536 102 158 108L182.249 150C185.713 156 181.385 163.5 174.249 163.5H125.751C118.615 163.5 114.287 156 117.751 150L142 108Z"
                    fill="#EF4444"
                  />
                  {/* Exclamation Bar */}
                  <line x1="150" y1="124" x2="150" y2="142" stroke="white" strokeWidth="4.5" strokeLinecap="round" />
                  {/* Exclamation Dot */}
                  <circle cx="150" cy="152" r="2.5" fill="white" />
                </g>
              </svg>
            </div>
          </div>
        ...
        <div className="w-full max-w-2xl bg-[#F0F7FF] border border-[#D0E3FF] rounded-2xl p-3.5 sm:p-5 flex flex-col xs:flex-row items-center gap-3 sm:gap-3.5 shadow-sm text-xs sm:text-sm text-[#475569] text-center xs:text-left">
          ...
          <p className="leading-relaxed">
            We're working to fix the issue as quickly as possible. Please check back in a few minutes.
          </p>
        </div>
